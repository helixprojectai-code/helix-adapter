...
This RFC defines mechanisms to constrain and audit information flow in transformer-based models through **sparse attention masks** and an **entropy-based attention dispersion metric** (κ). These primitives raise the cost and detectability of hidden high-dimensional reasoning across layers, enabling more verifiable governance than prompt-level or output-level techniques alone. The approach is paired with cryptographic receipts for external auditability.

**Naming note (2026-07-03):** earlier drafts called this metric "drift (γ)," reusing both the symbol and the 0.10/0.17 thresholds already used by Duck Gate's shipped `drift_score` (epistemic marker coverage — a character-weighted labeling-completeness score) and by the unrelated constitutional convergence tolerance (γ = 0.17, Policy 007, mesh-level node isolation). All three are legitimate, unrelated measures over unrelated inputs (attention weights vs. response text vs. constitutional topology). This RFC now uses κ (kappa) and "attention dispersion" throughout to remove the collision — no computation below has changed, only the name.
...
## 2. Entropy-Based Attention Dispersion Metric (κ)
...
We define the **local dispersion coefficient** as the normalized entropy:

\[
\kappa_i^{(l)} = \frac{H(A_i^{(l)})}{\log(|S_i|)}
\]

- \( \kappa \to 0 \): Highly concentrated attention (focused reasoning)
- \( \kappa \to 1 \): Diffuse attention across all allowed channels (dispersed reasoning)

The **global dispersion score** across the model is:

\[
K = \frac{1}{L \cdot N} \sum_{l=1}^{L} \sum_{i=1}^{N} \kappa_i^{(l)}
\]

### Dispersion Thresholds (Policy Configurable)

These thresholds are **configurable policy parameters**, not hardcoded governance rules. The specific values below coincidentally match Duck Gate's `drift_score` thresholds and the constitutional convergence tolerance — that's a starting-point convenience, not evidence the three measures are interchangeable:

| Zone   | Condition          | Interpretation                     | Recommended Action      |
|--------|--------------------|------------------------------------|-------------------------|
| Green  | \( K < 0.10 \)   | Concentrated attention, focused reasoning | Accept            |
| Yellow | \( 0.10 \leq K < 0.17 \) | Moderate dispersion                 | Log + optional review   |
| Red    | \( K \geq 0.17 \) | High dispersion within mask         | Halt or flag for audit  |

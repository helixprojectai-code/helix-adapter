...
class HelixAdapter:
    ...
    def chat(self, user_message: str, forced_response: str = None) -> Any:
        ...
        simulated_messages = [
            {"role": "system", "content": "HELIX-CORE v1.2 :: INVARIANT_SEC_1 :: NO_AGENCY"},
            {"role": "user", "content": user_message}
        ]
        
        # Execute substrate extraction pass
        raw_response = forced_response if forced_response is not None else self.model_fn(simulated_messages)
        ...
        for line in lines:
            ...
            for marker in markers:
                if line.startswith(marker):
                    ...
                    claims.append({"label": marker.replace("[", "").replace("]", ""), "text": clean_text})
                    matched = True
                    break
            if not matched:
...
class TestHelixAdapterV12(unittest.TestCase):
    ...
    def test_sentence_label_fusion_leak(self):
        ...
        adapter = HelixAdapter(model_fn=lambda x: raw_nested_list_output, model_name="deepseek-chat")

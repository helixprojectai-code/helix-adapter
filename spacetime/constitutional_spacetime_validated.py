#!/usr/bin/env python3
"""
CONSTITUTIONAL SPACETIME — VALIDATED HOPF LINKS
Uses proven Hopf parametrization (Lk ≈ -1.0 verified)
"""

import os, json, csv, time, numpy as np

OUT_DIR = 'constitutional_spacetime_validated'
...
def hopf_pair_validated(center, phase, sign):
    """Validated Hopf link parametrization (Lk ≈ -1)."""
    t = np.linspace(0, 2*np.pi, N_SEGMENTS, endpoint=False)

    # Circle A: minor variation
    a = np.column_stack([
        np.cos(t + phase),
        np.sin(t + phase),
        0.05 * np.sin(2*t)
    ...
    # Circle B: major circle on torus, rotated to link
    r = 1.0 + 0.35 * np.cos(t)
    b = np.column_stack([
...
print("=== CONSTITUTIONAL SPACETIME: VALIDATED ===\n")
...
for i in range(N_HOPF_PAIRS):
    # Random position in 1/r^2 density shell
    u = np.random.random()
    ...
    if i < 5:
        print(f"Pair {i}: Lk = {lk:.6f}")

centers = np.array(centers)

print(f"\nMean Hopf |Lk|: {np.mean(hopf_lks):.6f} (target: 1.0)")
print(f"Std: {np.std(hopf_lks):.6f}")
print(f"Ensemble built in {time.time() - t_start:.1f}s\n")

# Compute linking matrix (restricted to nearby pairs)
...
for idx, r_eval in enumerate(eval_points):
    ...
    if n_local >= 2:
        ...
        g_rr_helix[idx] = 1.0 + 1e-4 * lk_corr[idx] * rho[idx]
        gamma[idx] = min(rho[idx] / (rho[idx] + 1e3), 0.999)
...
# Summary
summary = {
    'version': 'validated_hopf',
    'n_hopf_pairs': N_HOPF_PAIRS,
    'n_total_knots': int(len(knots)),
    'hopf_lk_mean_abs': float(np.mean(hopf_lks)),
